...
//we separe in tiles that pthread can handle with the image
void smooth_filter_pthread(void *arg)
{
	args Arg = *(args*) arg;

	for(int x=Arg.x0; x<Arg.xm; x++)
	for(int y=Arg.y0; y<Arg.ym; y++)
	{
		Arg.vec2[_idx(x,y)] = smooth_filter(Arg.vec, x, y); 
	}
}
...
main(int argc, char **argv)
{
	...
	int  status, elem_per_thrd = nlin/num_threads;
	args args[num_threads];
	pthread_t threads[num_threads];
	
	// Loop de criacao das threads
	for (int i=0; i < num_threads; i++) {
		status = pthread_create(&threads[i], NULL, smooth_filter_pthread, (void *)&args[i]);
		if (status) {
